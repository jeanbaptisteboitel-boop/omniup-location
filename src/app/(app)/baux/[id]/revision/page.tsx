...
export default async function RevisionPage({ params }: { params: ParamsId }) {
  ...
  const b = await prisma.bail.findFirst({ where: { id, entiteId: await entiteCouranteId() }, include: { lot: true, locataire: true, revisions: { orderBy: { dateEffet: "desc" }, take: 1 } } });
  if (!b) notFound();
  ...
  return (
    <>
      <PageHeader titre="Révision du loyer" sousTitre={`${b.lot.nom} · ${nomComplet(b.locataire)}`} retour={{ href: `/baux/${b.id}`, libelle: "Bail" }} />
