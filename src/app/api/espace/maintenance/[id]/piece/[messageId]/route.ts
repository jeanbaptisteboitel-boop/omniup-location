...
/** Pièce jointe d'un message de maintenance, réservée au locataire auteur de la demande. */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string; messageId: string }> }) {
  ...
  const message = await prisma.messageMaintenance.findFirst({
    where: { id: Number(messageId) || 0, demandeId: Number(id) || 0, demande: { locataireId: l.id } },
