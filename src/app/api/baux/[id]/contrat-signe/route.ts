...
/** Exemplaire signé du contrat (gestionnaire). */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const bail = await prisma.bail.findFirst({ where: { id: Number(id) || 0, entiteId: await entiteCouranteId() }, select: { contratSigneChemin: true, contratSigneNom: true, contratSigneMime: true } });
