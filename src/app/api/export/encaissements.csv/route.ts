...
import { MODES_PAIEMENT, nomComplet } from "@/lib/libelles";
...
export async function GET(req: NextRequest) {
  ...
  const paiements = await prisma.paiement.findMany({
    ...
    include: { appel: { include: { bail: { include: { lot: { include: { bailleur: true } }, locataire: true } } } } },
    orderBy: { date: "asc" },
  });
  const csv = versCSV(
    ...
    paiements.map((p) => {
      ...
      return [formatDate(p.date), p.appel.bail.lot.nom, p.appel.bail.lot.bailleur?.nom ?? "", nomComplet(p.appel.bail.locataire), formatPeriode(p.appel.periode), numeroAppel(p.appel.id), MODES_PAIEMENT[p.mode], p.reference, p.montant, partLoyer, arrondir2(p.montant - partLoyer)];
    }),
  );
