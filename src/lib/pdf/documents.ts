...
export async function pdfContrat(bail: { type: keyof typeof TYPES_BAIL; texteContrat: string | null; dateDebut: Date; lot: Lot & { bailleur: Bailleur | null }; locataires: Locataire[] }): Promise<Buffer> {
...
export async function pdfCourrier(courrier: { objet: string; contenu: string; createdAt: Date; bail: { lot: Lot & { bailleur: Bailleur | null }; locataires: Locataire[] } }): Promise<Buffer> {
  ...
  if (!dejaEnTete) {
    ...
    blocDestinataire(doc, lignesLocataire(courrier.bail.locataires, courrier.bail.lot));
